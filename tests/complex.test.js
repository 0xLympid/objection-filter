    ...
    describe(knexConfig.client, function () {
      ...
      describe('edge cases', function () {
        ...
        it('should be equivalent if require/where on a root model column', async () => {
          const results1 = await buildFilter(Person).build({
            require: {
              firstName: 'F01',
            },
          });

          const results2 = await buildFilter(Person).build({
            where: {
              firstName: 'F01',
            },
          });
      ...
      describe('optimization', function () {
        context('given require filter is purely belongsTo', () => {
          let query;
          beforeEach(() => {
            query = buildFilter(Animal).build({
              require: {
                'owner.firstName': 'F00',
              },
              eager: 'owner',
        ...
        context('given require filter is purely belongsToOne', () => {
          let query;
          beforeEach(() => {
            query = buildFilter(Person).build({
              require: {
