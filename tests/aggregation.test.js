    ...
    describe(knexConfig.client, function () {
      ...
      describe('defaults', function () {
        it('should skip if aggregations = []', async () => {
          ...
          result
            .map((item) => item.count)
            .should.deep.equal(fillArray([10], [undefined]));
        });
      ...
      describe('count without filters', function () {
        ...
        it('should count using 1-level belongsTo', async () => {
          const result = await buildFilter(Animal).build({
            aggregations: [
              {
                type: 'count',
                relation: 'owner',
      ...
      describe('count with filters', function () {
        it('should count using 1-level manyToMany', async () => {
          ...
          result
            .map((item) => item.count)
            .sort(NUMERIC_SORT)
            .should.deep.equal(fillArray([2, 8], [0, 10]));
        });
      });
      ...
      describe('composite ids', function () {
        it('should aggregate if root model has composite id', async function () {
          const result = await buildFilter(MovieVersion).build({
            aggregations: [
              {
                type: 'count',
                relation: 'movie',
          ...
          result
            .map((item) => item.count)
            .should.deep.equal(fillArray([100], [1]));
        });
        ...
        it('should aggregate if outer model has composite id', async () => {
          ...
          await MovieVersion.query().delete().where({ movieId: 1 });
          ...
          result
            .map((item) => item.count)
            .should.deep.equal(fillArray([1, 99], [0, 1]));
        });
      });
      ...
      describe('multiple aggregations', function () {
        it('should apply multiple counts', async () => {
          ...
          const counts = result.map(({ count1, count2 }) => ({
            count1,
            count2,
          }));
          counts.should.deep.equal(
            fillArray([100], [{ count1: 1, count2: 10 }]),
          );
        });
      ...
      describe('integration', function () {
        ...
        it('should allow ordering based on the aggregated field', async () => {
          ...
          result
            ...
            .should.deep.equal(['F09', 'F08', 'F07', 'F06', 'F05']);
        });
      });
      ...
      describe('onAggBuild', function () {
        it('should return zero owners when filtered', async () => {
          const onAggBuild = function (RelatedModelClass) {
            return RelatedModelClass.query().where({ id: 34343 });
          };

          const result = await buildFilter(Animal, null, { onAggBuild }).build({
            aggregations: [
              {
                type: 'count',
                relation: 'owner',
        ...
        it('should return zero for all owners where firstName !== "F00"', async () => {
          const onAggBuild = function (RelatedModelClass) {
            if (RelatedModelClass.name === 'Person') {
              return RelatedModelClass.query().where({ firstName: 'F00' });
            }
          };
