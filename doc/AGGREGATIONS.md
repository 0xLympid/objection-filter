...
Aggregations can currently be added to the base model (not onto loaded models).
...
This is a problem that can occur during loading, but also aggregation. Take the following as an example, for a multi-tenant Shop management platform:
...
const createOnAggBuild = (tenantId) => (Model) => {
  if (Model.name === 'Shop') return Model.query().where({ tenantId });
};

// Route handler for GET /ShopTypes
async function getShopTypes(req) {
  const { tenantId } = req.decoded; // Using some express middleware
  return await buildFilter(ShopType, null, {
    onAggBuild: createOnAggBuild(tenantId),
  })
    .build(req.query)
    .onBuild(sliceByTenant(tenantId));
}
